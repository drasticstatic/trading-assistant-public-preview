...
Feb 25 was the A+ session — double green day on MNQ (+$565) and SOL (+$20.63 USDT). The SOL trade on BTCC ran in parallel with the MNQ hold and confirmed the same thesis: IT Foundation EMA crossover green dominant + FVG pullback = long entry. SOL/USDT entered near the session low and exited near the HOD without intervention, riding a +$4.09 move over 2.5 hours.
...
1. **Continue applying IT Foundation framework outside NY AM session on crypto.** The EMA crossover + FVG pullback setup is proven on SOL. When the signal appears, evaluate it on its merits — independent of whether the futures session is also active.
2. **BTCC account now has a positive balance — manage it as a real account.** Real risk management rules apply: position size, SL placement, no overleverage beyond the $20 voucher model.
3. **The edge is portable — use the same structural criteria everywhere.** Both MNQ and SOL validated the same framework on the same day. When the five-layer stack appears on any instrument, that is the trade.
