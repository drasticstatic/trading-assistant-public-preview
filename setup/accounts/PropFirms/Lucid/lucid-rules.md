...
**Copy-trading plan (active):** **leads** Group A (Strict News Discipline) via [TradeCopia](https://tradecopia.com), with **TPT following at 2x** (flipped Sep 3, 2026 — TPT led originally). Full current plan: `specs/TradeCopia_workflow.md`. This account (connected as **LucidFlex25**, `LFE02590489830001`) is the PropFirmMatchTV duck-race prize — PFM confirmed the granted style is Flex, not Daily. The group was originally TPT-led (its PRO Account Rules #6 news-blackout obligation gated every trade before it reached either account), then flipped to Lucid-led specifically because Christopher typically trades a single micro contract per entry, and a fractional multiplier scaling down from a 1-contract leader trade isn't a valid position — leading from the smaller account and scaling up avoids that. TPT is still bound by its own news rule as a follower; Christopher is covering that gap by trading both leaders manually and independently. No compliance issue either direction — neither TPT's nor Lucid's rules restrict which account leads. Selected the **$600 DLL** option on setup, since overridden by the personal $555 daily-loss config applied to all six TradeCopia accounts (see workflow spec). Lucid's own rules already permit trade copiers/automation, including as a leader, with no approval step (line 109, 413 above).
