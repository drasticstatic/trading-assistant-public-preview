...
**No Lucid account currently held, but a free account was won** via a PropFirmMatchTV ([propfirmmatch.com/futures](https://propfirmmatch.com/futures)) on-stream duck race giveaway 🦆 (Sep 2026) — not yet purchased/active. **Correction from an earlier draft of this note:** this is not confirmed as specifically LucidFlex — support's message described it as a **free 25K account**, redeemable via a one-time code toward purchasing a 25K Futures account, with the account **style still open**. Christopher confirmed with support that any of the 25K-style Lucid accounts (Flex/Daily/Pro) can be selected with the code; asked a follow-up about Flex vs Pro vs Daily and DLL options, awaiting reply as of Sep 2, 2026. Full comparison built to help decide: [lucid-account-types-comparison.md](lucid-account-types-comparison.md).

**Copy-trading plan (active):** copy-trades from **TPT** via [TradeCopia](https://tradecopia.com), as **Group A (Strict News Discipline)** — not TopOneFutures. Full current plan: `specs/TradeCopia_workflow.md`. This superseded an earlier draft that paired this account with TopOneFutures Elite ACCESS as the EOD leader — TopOneFutures' own AI assistant confirmed that pairing (50K leader, 25K follower) is a policy violation on account-size grounds, so the account moved to TPT's group instead, where neither firm's rules impose a size-matching restriction. **PFM confirmed (Sep 3, 2026) the actual granted style is LucidFlex, not LucidDaily** — connected in TradeCopia as **LucidFlex25**. Christopher kept it in TPT's group anyway for mental-model simplicity, even though LucidFlex doesn't share LucidDaily's hard-breach news restriction (news trading is generally allowed on Flex — see line 106 above) and runs EOD drawdown, a genuine mechanic mismatch with TPT's intraday. That's fine under the group structure's actual rule (no mechanic-matching requirement, and neither TPT's nor Lucid's rules impose a size constraint) — just don't assume this account inherits TPT's news-blackout discipline. Selected the **$600 DLL** option on setup (skipped the +$10 no-DLL upgrade) — that hard daily-loss ceiling directly constrains how the TPT→LucidFlex25 quantity multiplier gets sized. Lucid's own rules already permit trade copiers/automation with no approval step (line 109, 413 above) — no Lucid-side restriction blocks this.
