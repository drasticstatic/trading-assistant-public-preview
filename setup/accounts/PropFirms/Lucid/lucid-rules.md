...
**Copy-trading plan (once active):** intended to copy-trade from TopOneFutures Elite ACCESS via [TradeCopia](https://tradecopia.com) (matching EOD-style accounts) — TopOneFutures itself stays manual. Full plan: `setup/accounts/PropFirms/prop-firm-Scaling-SOP-&-Implementation.md`'s TradeCopia section. This EOD-to-EOD pairing is exactly why [lucid-account-types-comparison.md](lucid-account-types-comparison.md) leans toward LucidFlex or LucidPro over LucidDaily (funded LucidDaily is always intraday drawdown, which mismatches the EOD leader). Confirm this doesn't conflict with any Lucid-side copy-trading restriction before activating — the general rules above don't currently document a Lucid-specific copy-trading policy; verify with Lucid's own terms once the account is live.
