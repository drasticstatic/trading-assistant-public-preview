...
**Copy-trading plan (once active):** intended to copy-trade from **TPT** via [TradeCopia](https://tradecopia.com), as **Group A** — not TopOneFutures. Full current plan: `specs/TradeCopia_workflow.md`. This superseded an earlier draft that paired this account with TopOneFutures Elite ACCESS as the EOD leader — TopOneFutures' own AI assistant confirmed that pairing (50K leader, 25K follower) is a policy violation on account-size grounds, so the account moved to TPT's group instead, where neither firm's rules impose a size-matching restriction. This also resolves [lucid-account-types-comparison.md](lucid-account-types-comparison.md)'s lean toward LucidFlex/LucidPro for an EOD match — with TPT (intraday) as the leader, **LucidDaily** (also intraday) is now the better fit; Flex/Pro would only make sense here if a future account happens to size-match Top1 (see `specs/TradeCopia_workflow.md`). Lucid's own rules already permit trade copiers/automation with no approval step (line 109, 413 above) — no Lucid-side restriction blocks this.
