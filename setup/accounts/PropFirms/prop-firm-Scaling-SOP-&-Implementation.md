...
**Group A (Strict News Discipline) — led by LucidFlex25 (25K, flipped Sep 3, 2026 — was led by TPT)**
- Follower: **TPT** (the 50K account, `TAKEPROFITPRO781112132`), 2x multiplier. Flipped from the original TPT-led structure specifically to solve a whole-number problem: Christopher typically trades 1 micro contract per entry, and a fractional multiplier scaling down from a 1-contract leader trade isn't a valid position. Leading with the smaller account and scaling up avoids that entirely. Traded off deliberately: TPT led originally because its PRO Account Rules #6 news-blackout obligation (see `TakeProfitTrader/tpt-rules.md`) gated every trade before it could reach either account; as a follower, TPT is still bound by that same rule, so the gate is no longer structural — Christopher is covering that gap with his own manual discipline (he trades both leaders independently, TPT and Top1, honoring each firm's own rules directly). Full rationale: `specs/TradeCopia_workflow.md`.
