...
> **Correction, Sep 2026 — the "no maximum time limit" language is accurate, but only while the subscription keeps renewing.** Straight from TPT's own support AI (Mira), asked directly after a real account ended up disabled:
> > "If you don't reach the profit target within 30 days, your test does not automatically end. You may continue working toward it, and the subscription renews every 30 days until you cancel it. A subscription can include multiple attempts or resets under the same journey... If you already canceled your subscription, it should not renew or charge you again. Your test account remains available until the end of the current subscription period, after which it expires if you have not passed."
>
> So the real mechanic: a test/eval subscription auto-renews every 30 days by default, and each renewal carries the account's progress forward — genuinely "no maximum time limit" as long as billing keeps going, the same carry-forward behavior Apex Legacy renewals have (see `prop-firm-rules.html`'s Reset Economics section). **What actually disables an account is canceling the subscription** — once canceled, the account simply runs out at the end of its current already-paid period and expires/disables permanently, whether or not the profit target was ever hit. This is exactly what happened on one account: the subscription was canceled for lack of liquidity to renew (a different deal, 50AND3, was chosen for the next attempt instead of renewing under NOFEE40 — both come fully activated either way), and that account is now permanently disabled rather than reset. One side effect worth knowing: a passed-then-expired test still shows as "disabled" and its data stays visible in the dashboard, but a *failed* test's data appears to get wiped entirely — pull any PDF export or screenshots you want to keep before letting a subscription lapse.
>
> Separately and unrelated to the above: TPT's [Rules for Multiple Accounts](https://takeprofittraderhelp.zendesk.com/hc/en-us/articles/15172695563933-Rules-for-Multiple-Accounts) article also caps active PRO/PRO+ accounts at 5 and passed-test activations at 10 per rolling 30 calendar days — a queue-management rule for traders running several accounts at once, not what caused the disablement above, but worth knowing if running more than a couple of PRO/PRO+ accounts simultaneously.
