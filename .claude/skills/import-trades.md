...
3. Note the export type:
   ...
   - **BTCC:** normally "Trade History" export; if the download doesn't work, Christopher may hand over a browser-copy-paste routed through Apple Numbers (a `.numbers` file with a `.csv` sibling — read the `.csv`). A blank starting-point Numbers file for this is archived at `data/imports/templates/btcc-browser-copy-template.numbers` (kept outside the dated YYYY/MM-Mon tree since it's a reusable template, not an archived export) — reuse it as the template rather than building a fresh spreadsheet each time.
